...
describe('getEntry', () => {
  it('should return the entry when it exists', async () => {
    ...
    const created = await createEntry(repository, { text: 'contenu', tag: 'dev', mediaUrls: [], audioUrls: [] }, fixedClock)
  ...
  it('should find an entry by its exact slug', async () => {
    const repository = new InMemoryEntryRepository()
    await createEntry(repository, { text: 'first', tag: null, mediaUrls: [], audioUrls: [] }, fixedClock)
    const created = await createEntry(repository, { text: 'target', tag: 'dessin', mediaUrls: [], audioUrls: [] }, () => new Date(2026, 3, 29, 15, 0, 0))
    const result = await getEntry(repository, created.slug)
    expect(result.ok).toBe(true)
    if (result.ok) {
