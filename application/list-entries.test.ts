...
describe('listEntries', () => {
  ...
  it('should return all created entries', async () => {
    const repository = new InMemoryEntryRepository()
    await createEntry(repository, { text: 'first', tag: null, mediaUrls: [] }, () => new Date(2026, 3, 28))
    await createEntry(repository, { text: 'second', tag: 'dev', mediaUrls: [] }, () => new Date(2026, 3, 29))
    const entries = await listEntries(repository)
  ...
  it('should return entries in reverse chronological order', async () => {
    const repository = new InMemoryEntryRepository()
    await createEntry(repository, { text: 'older', tag: null, mediaUrls: [] }, () => new Date(2026, 3, 28))
    await createEntry(repository, { text: 'newer', tag: null, mediaUrls: [] }, () => new Date(2026, 3, 29))
    const entries = await listEntries(repository)
